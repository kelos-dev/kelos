...
- **E2E tests for cross-agent features must be parameterized over all agent types.** When a feature flows through the shared `Task` / `AgentConfig` surface and is not tied to one CLI's implementation details (e.g. plugin skills, skills.sh packages, model overrides, prompt template variables), write the test as a function that takes an `agentTestConfig` and iterate it over the `agentConfigs` slice (see `describePluginTaskTests` in `test/e2e/skills_test.go`). An e2e test that only exercises one agent — e.g. only `claude-code` or only `codex` — silently masks regressions in the other agents, because the controller path the feature exercises is shared.
- **Prefer agent-native shorthand in docs and example YAMLs for fields the CLI accepts in shorthand form.** For `spec.model` and `taskTemplate.model`, show the shorthand (e.g. `model: sonnet`, `model: opus` for Claude Code) as the primary example, and mention the versioned form (e.g. `claude-sonnet-4-6`) as an alternative. Versioned IDs go stale every model release and need to be swept across docs each time; the shorthand stays current and reads as the idiomatic form a CLI user would actually type.

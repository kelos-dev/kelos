...
const (
	// TaskPipelineConditionSucceeded reports whether every pipeline stage succeeded.
	TaskPipelineConditionSucceeded = "Succeeded"
)
...
// as .Stages, keyed by stage name. Each value lists that stage's Task results
// in index order, with Name, Matrix, Results, and Outputs fields.
//
...
type PipelineStage struct {
	...
	// TaskTemplate defines the Tasks created for this stage. Prompt and branch
	// templates receive .Matrix and .Stages. Each .Stages value lists that
	// stage's Task results in index order, with Name, Matrix, Results, and Outputs fields.
...
type PipelineStageStatus struct {
	...
	// Phase uses the Task phase vocabulary to summarize this stage's Tasks.
	// +optional

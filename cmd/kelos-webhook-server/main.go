...
func main() {
	...
	// Validate source parameter only when the server routes by source.
	...
	// (/webhook/<namespace>/<name>) using WebhookGateway resources. Otherwise, a
	// source-specific handler is mounted (generic uses /webhook/<source>, while
	// GitHub and Linear use the root path).

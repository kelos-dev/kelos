...
| `spec.when.gitlab.baseUrl` | GitLab instance URL for API calls (e.g., `https://gitlab.example.com` or an in-cluster service URL). Defaults to the scheme and host of the workspace repo URL | No |
...
| `spec.gitlab.apiBaseURL` | GitLab instance URL used for status notes (e.g. `https://gitlab.example.com` or an in-cluster Service URL). Defaults to the instance URL taken from the webhook payload | No |

...
- **Do not set `excludeAuthors`:** the sender of a `check_run` event is the CI
  ...
  `conclusion` and `checkName` filters instead. A `pullRequestAuthor`
  criterion does not help here either: GitHub's `check_run.pull_requests[]`
  entries carry no `user`, so a `check_run` event has no pull request author to
  match against and never matches that criterion.

...
func TestBuildJob_MarketplacePlugins(t *testing.T) {
	builder := NewJobBuilder()
	task := &kelosv1alpha1.Task{
		ObjectMeta: metav1.ObjectMeta{Name: "test-marketplace", Namespace: "default"},
		Spec: kelosv1alpha1.TaskSpec{
			Type:   AgentTypeClaudeCode,
			Prompt: "Fix issue",
			Credentials: kelosv1alpha1.Credentials{
	...
	mc := job.Spec.Template.Spec.Containers[0]
	found := false
	for _, env := range mc.Env {
		if env.Name == "KELOS_MARKETPLACE_PLUGINS" {
			found = true
			if env.Value != "github@anthropics-claude-code,commit-commands@claude-plugins-official" {
				t.Errorf("Expected comma-separated marketplace plugins, got: %q", env.Value)
			}
		}
	}
	if !found {
		t.Error("Expected KELOS_MARKETPLACE_PLUGINS env var on main container")
	}
}

func TestBuildJob_MarketplacePluginsCombinedWithGitHub(t *testing.T) {
	builder := NewJobBuilder()
	task := &kelosv1alpha1.Task{
		ObjectMeta: metav1.ObjectMeta{Name: "test-combined-mp", Namespace: "default"},
		Spec: kelosv1alpha1.TaskSpec{
			Type:   AgentTypeClaudeCode,
			Prompt: "Fix issue",
			Credentials: kelosv1alpha1.Credentials{
	...
	agentConfig := &kelosv1alpha1.AgentConfigSpec{
		MarketplacePlugins: []string{"github@anthropics-claude-code"},
		Plugins: []kelosv1alpha1.PluginSpec{
			{
				Name:   "my-plugin",
				GitHub: &kelosv1alpha1.GitHubPluginSource{Repo: "acme/tools"},
	...
	foundMPPlugins := false
	foundPluginDir := false
	for _, env := range mc.Env {
		if env.Name == "KELOS_MARKETPLACE_PLUGINS" {
			foundMPPlugins = true
		}
		if env.Name == "KELOS_PLUGIN_DIR" {
			foundPluginDir = true
		}
	}
	if !foundMPPlugins {
	...
	if !foundPluginDir {
		t.Error("Expected KELOS_PLUGIN_DIR env var on main container")
	}

	// Should have init container for GitHub plugin setup.
	if len(job.Spec.Template.Spec.InitContainers) != 1 {
		t.Fatalf("Expected 1 init container for GitHub plugin, got %d", len(job.Spec.Template.Spec.InitContainers))
	}
}

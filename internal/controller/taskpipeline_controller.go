...
const (
	...
	taskPipelineStageLabel = "kelos.dev/taskpipeline-stage"
	taskPipelineIndexLabel = "kelos.dev/taskpipeline-index"
				...
				if err := r.Create(ctx, task); err != nil {
					if apierrors.IsAlreadyExists(err) {
						...
						case metav1.IsControlledBy(&existingTask, &pipeline) &&
							existingTask.Labels[taskPipelineStageLabel] == stage.Name &&
							existingTask.Labels[taskPipelineIndexLabel] == strconv.Itoa(index):
...
func (r *TaskPipelineReconciler) failInvalidPipeline(ctx context.Context, pipeline *kelos.TaskPipeline, validationErr error) error {
	...
	setTaskPipelineSucceededCondition(pipeline, metav1.ConditionFalse, "InvalidPipeline", validationErr.Error())
...
func (r *TaskPipelineReconciler) listPipelineTasks(ctx context.Context, pipeline *kelos.TaskPipeline) (map[string][]*kelos.Task, error) {
	...
	for i := range taskList.Items {
		...
		stageName := task.Labels[taskPipelineStageLabel]
		if stageName == "" {
			return nil, fmt.Errorf("Task %q owned by TaskPipeline %q has no %s label", task.Name, pipeline.Name, taskPipelineStageLabel)
		}
	...
	task := &kelos.Task{
		ObjectMeta: metav1.ObjectMeta{
			...
			Labels: map[string]string{
				...
				taskPipelineStageLabel: stage.Name,
				taskPipelineIndexLabel: strconv.Itoa(index),
			},
		},
		Spec: kelos.TaskSpec{
			Prompt: prompt,
...
func pipelineTaskIndex(task *kelos.Task) int {
	index, err := strconv.Atoi(task.Labels[taskPipelineIndexLabel])
	if err != nil {
		return -1
	}
	return index
...
func (r *TaskPipelineReconciler) updateTaskPipelineStatus(
	...
	case allSucceeded:
		...
		setTaskPipelineSucceededCondition(pipeline, metav1.ConditionTrue, "Succeeded", "All pipeline stages succeeded")
	case failure != "" && active == 0:
		pipeline.Status.Phase = kelos.TaskPipelinePhaseFailed
		setTaskPipelineSucceededCondition(pipeline, metav1.ConditionFalse, "Failed", failure)
	case observed > 0:
		...
		setTaskPipelineSucceededCondition(pipeline, metav1.ConditionUnknown, "Running", message)
	default:
		pipeline.Status.Phase = kelos.TaskPipelinePhasePending
		setTaskPipelineSucceededCondition(pipeline, metav1.ConditionUnknown, "Pending", "Pipeline has not started")
	}
...
func setTaskPipelineSucceededCondition(
	pipeline *kelos.TaskPipeline,
	...
	apiMeta.SetStatusCondition(&pipeline.Status.Conditions, metav1.Condition{
		Type:               kelos.TaskPipelineConditionSucceeded,
		Status:             status,

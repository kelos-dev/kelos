...
func TestTaskPipelineReconcileCreatesMatrixTasks(t *testing.T) {
	...
	for i := range tasks.Items {
		...
		if task.Labels[taskPipelineLabel] != pipeline.Name || task.Labels[taskPipelineStageLabel] != "scan" {
...
func TestTaskPipelineReconcilePassesStageResults(t *testing.T) {
	...
	for i := range tasks.Items {
		if tasks.Items[i].Labels[taskPipelineStageLabel] == "report" {
	...
	succeeded := apiMeta.FindStatusCondition(updated.Status.Conditions, kelos.TaskPipelineConditionSucceeded)
	if succeeded == nil || succeeded.Status != metav1.ConditionTrue {
		t.Fatalf("Succeeded condition = %#v, want True", succeeded)
	}
}
...
func TestTaskPipelineReconcileFailsOnMissingResult(t *testing.T) {
	...
	succeeded := apiMeta.FindStatusCondition(updated.Status.Conditions, kelos.TaskPipelineConditionSucceeded)
	if succeeded == nil || !strings.Contains(succeeded.Message, `key "branch" is not present`) {
		t.Fatalf("Succeeded condition = %#v", succeeded)
	}
}

func TestTaskPipelineReconcileWaitsForActiveTasksBeforeFailing(t *testing.T) {
	pipeline := testTaskPipeline("failed-task", []kelos.PipelineStage{
		{
			Name: "test",
			Matrix: &kelos.PipelineMatrix{Items: []map[string]string{
				{"package": "api"},
			...
			TaskTemplate: testPipelineTaskTemplate("Test {{index .Matrix \"package\"}}"),
		},
		{Name: "publish", TaskTemplate: testPipelineTaskTemplate("Publish the results")},
	})
	reconciler, k8sClient := testTaskPipelineReconciler(t, pipeline)
	reconcileTaskPipeline(t, reconciler, pipeline)
	...
	failedTask := &tasks.Items[0]
	activeTask := &tasks.Items[1]
	if pipelineTaskIndex(failedTask) != 0 {
		failedTask, activeTask = activeTask, failedTask
	}
	failedTask.Status.Phase = kelos.TaskPhaseFailed
	failedTask.Status.Message = "tests failed"
	if err := k8sClient.Status().Update(context.Background(), failedTask); err != nil {
		t.Fatal(err)
	}
	...
	updated := getTaskPipeline(t, k8sClient, pipeline.Name)
	if updated.Status.Phase != kelos.TaskPipelinePhaseRunning {
		t.Fatalf("pipeline phase = %q, want %q while a sibling Task is active", updated.Status.Phase, kelos.TaskPipelinePhaseRunning)
	}
	succeeded := apiMeta.FindStatusCondition(updated.Status.Conditions, kelos.TaskPipelineConditionSucceeded)
	wantFailure := fmt.Sprintf("Stage %q failed because Task %q failed: tests failed", "test", failedTask.Name)
	if succeeded == nil || succeeded.Status != metav1.ConditionUnknown || succeeded.Message != wantFailure {
		t.Fatalf("Succeeded condition = %#v, want Unknown with message %q", succeeded, wantFailure)
	}
	if err := k8sClient.List(context.Background(), &tasks, client.InNamespace("default")); err != nil {
		t.Fatal(err)
	}
	if len(tasks.Items) != 2 {
		t.Fatalf("created Tasks after child failure = %d, want 2", len(tasks.Items))
	}

	activeTask.Status.Phase = kelos.TaskPhaseSucceeded
	if err := k8sClient.Status().Update(context.Background(), activeTask); err != nil {
		t.Fatal(err)
	}
	reconcileTaskPipeline(t, reconciler, pipeline)
	...
	succeeded = apiMeta.FindStatusCondition(updated.Status.Conditions, kelos.TaskPipelineConditionSucceeded)
	if succeeded == nil || succeeded.Status != metav1.ConditionFalse || succeeded.Message != wantFailure {
		t.Fatalf("Succeeded condition = %#v, want False with message %q", succeeded, wantFailure)
	}
}
...
func TestTaskPipelineReconcileWaitsForEarlierPipelineTasks(t *testing.T) {
	...
	oldTask := &kelos.Task{ObjectMeta: metav1.ObjectMeta{
		...
		Labels: map[string]string{
			...
			taskPipelineStageLabel: "work",
			taskPipelineIndexLabel: "0",
		},
...
func TestTaskPipelineReconcileRejectsChildTaskNameCollision(t *testing.T) {
	pipeline := testTaskPipeline("collision", []kelos.PipelineStage{
		{
			Name: "review",
			Matrix: &kelos.PipelineMatrix{Items: []map[string]string{
				{"focus": "api"},
			...
			TaskTemplate: testPipelineTaskTemplate("Review {{index .Matrix \"focus\"}}"),
		},
		{Name: "review-0", TaskTemplate: testPipelineTaskTemplate("Summarize the review")},
	})
	reconciler, k8sClient := testTaskPipelineReconciler(t, pipeline)
	reconcileTaskPipeline(t, reconciler, pipeline)
	...
	succeeded := apiMeta.FindStatusCondition(updated.Status.Conditions, kelos.TaskPipelineConditionSucceeded)
	collidingName := pipelineChildTaskName(pipeline.Name, "review-0", 0, 1)
	wantFailure := fmt.Sprintf("Stage %q failed: Task name %q is already in use", "review-0", collidingName)
	if succeeded == nil || succeeded.Status != metav1.ConditionFalse || succeeded.Message != wantFailure {
		t.Fatalf("Succeeded condition = %#v, want False with message %q", succeeded, wantFailure)
	}
	if err := k8sClient.List(context.Background(), &tasks, client.InNamespace("default")); err != nil {
	...
	if len(tasks.Items) != 2 {
		t.Fatalf("created Tasks after name collision = %d, want 2", len(tasks.Items))
	}
	for i := range tasks.Items {
		if tasks.Items[i].Labels[taskPipelineStageLabel] == "review-0" {
			t.Fatalf("Task %q was incorrectly attributed to the colliding stage", tasks.Items[i].Name)
		}
	}
}
...
func TestPipelineMatrixItemsRejectsTooManyTasks(t *testing.T) {
	...
	wantMessage := fmt.Sprintf("maximum of %d items", maxPipelineStageTasks)

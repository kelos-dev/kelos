...
// Dispatch: if a WorkerPool exists with req.Name it takes priority. If a
// WorkerPool and Task share the same name in the same namespace, only the
// pool is reconciled — this is acceptable since task watches enqueue the
// task's own name (which won't collide with a pool it references).
func (r *WorkerPoolReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	switch {
	case poolErr == nil:
		return r.reconcilePool(ctx, &pool)
...
func (r *WorkerPoolReconciler) reconcileTask(ctx context.Context, task *kelos.Task) (ctrl.Result, error) {
	...
	// Handle deletion: clear the pod assignment so the worker becomes available.
	// This is a safety net for event-based reconciles; TaskReconciler.handleDeletion
	// also clears the assignment via the finalizer path.
		...
		if task.Status.PodName != "" {
			...
			if err := r.Get(ctx, types.NamespacedName{Name: task.Status.PodName, Namespace: task.Namespace}, &pod); err == nil {
				if err := r.clearPodAssignment(ctx, &pod); err != nil {
					logger.Error(err, "Failed to clear pod assignment for deleted task", "pod", pod.Name, "task", task.Name)
				}
			}
		}
		return ctrl.Result{}, nil
...
func (r *WorkerPoolReconciler) completeTask(ctx context.Context, task *kelos.Task, phase kelos.TaskPhase, message string) error {
	outputs, results := r.readPodOutputs(ctx, task.Namespace, task.Status.PodName)
	return retry.RetryOnConflict(retry.DefaultRetry, func() error {
		if err := r.Get(ctx, client.ObjectKeyFromObject(task), task); err != nil {
			return err
		}
...
func (r *WorkerPoolReconciler) readPodOutputs(ctx context.Context, namespace, podName string) ([]string, map[string]string) {
...
func (r *WorkerPoolReconciler) clearPodAssignment(ctx context.Context, pod *corev1.Pod) error {
	...
	return r.Patch(ctx, pod, podPatch)
}

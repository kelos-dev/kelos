...
func (r *Runner) Run(ctx context.Context) error {
	...
	for {
		...
		if err := r.waitForAnnotationClear(ctx); err != nil {
			return fmt.Errorf("waiting for annotation clear: %w", err)
		}
...
func (r *Runner) executeTask(ctx context.Context, taskName string) error {
	...
	if err != nil {
		...
		_ = r.setTaskStatus(ctx, "failed", reason)
		return fmt.Errorf("getting task %s: %w", taskName, err)
	}

	if err := r.runAgentFunc(ctx, task); err != nil {
		reason := fmt.Sprintf("agent execution failed: %v", err)
		_ = r.setTaskStatus(ctx, "failed", reason)
		return err
	}

	if err := r.setTaskStatus(ctx, "succeeded", ""); err != nil {
...
func (r *Runner) setTaskStatus(ctx context.Context, status, failureReason string) error {
...
func (r *Runner) waitForAnnotationClear(ctx context.Context) error {
	for {
		select {
		case <-ctx.Done():
			return ctx.Err()

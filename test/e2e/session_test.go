...
var _ = Describe("Session remote control", func() {
	...
	It("shares one provider session across clients and resets its workspace", func() {
		...
		Expect(err).NotTo(HaveOccurred())
		_ = connection.Close()
		updateSessionSuspend(f, f.Namespace, sessionName, true)
		...
		updateSessionSuspend(f, f.Namespace, sessionName, false)
...
func updateSessionSuspend(f *framework.Framework, namespace, name string, suspend bool) {
	Eventually(func() error {

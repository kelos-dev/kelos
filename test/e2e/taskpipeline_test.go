...
Reply with exactly "complete".`,
		...
		Eventually(func() []string {
			tasks, err := f.KelosClientset.ApiV1alpha2().Tasks(f.Namespace).List(context.TODO(), metav1.ListOptions{
				LabelSelector: "kelos.dev/taskpipeline=matrix-pipeline,kelos.dev/taskpipeline-stage=review",
			})
			if err != nil {
				return nil
			}
		...
		succeeded := apiMeta.FindStatusCondition(pipeline.Status.Conditions, kelos.TaskPipelineConditionSucceeded)

...
var _ = Describe("AgentConfig conversion webhook", Ordered, func() {
	...
	It("preserves TaskSpawner slack excludeFilters across a v1alpha1 round-trip", func() {
		...
		By("Creating a v1alpha2 TaskSpawner with slack channels and an exclusion rule")
			...
			Spec: kelos.TaskSpawnerSpec{
				When: kelos.When{
					Slack: &kelos.Slack{
						...
						ExcludeFilters: []kelos.SlackFilter{{Channels: []string{"C9876543210", "D0123456789"}}},
					},
				},
		...
		Expect(down.Annotations).To(HaveKeyWithValue(
			"kelos.dev/v1alpha2-slack-exclude-filters",
			`[{"channels":["C9876543210","D0123456789"]}]`))
		...
		By("Reading it as v1alpha2 and asserting the exclusion rule was restored")
		got := &kelos.TaskSpawner{}
		...
		Expect(got.Spec.When.Slack.ExcludeFilters).To(Equal([]kelos.SlackFilter{
			{Channels: []string{"C9876543210", "D0123456789"}},
		}))
		Expect(got.Annotations).NotTo(HaveKey("kelos.dev/v1alpha2-slack-exclude-filters"))

		By("Rejecting an exclusion rule with no criteria, which would match every message")
		empty := &kelos.TaskSpawner{
			ObjectMeta: metav1.ObjectMeta{Name: "ts-slack-empty-rule", Namespace: key.Namespace},
			Spec: kelos.TaskSpawnerSpec{
				When: kelos.When{
					Slack: &kelos.Slack{ExcludeFilters: []kelos.SlackFilter{{}}},
				},
				TaskTemplate: kelos.TaskTemplate{
					Type:         "claude-code",
					Credentials:  &kelos.Credentials{Type: kelos.CredentialTypeNone},
